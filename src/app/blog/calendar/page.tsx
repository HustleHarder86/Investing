...
export const metadata: Metadata = {
  title: 'Blog Content Calendar | Money in Motion',
  description: "Upcoming blog posts and content schedule for Money in Motion financial planning blog.",
...
const WrittenBadge = ({ isWritten }: { isWritten: boolean }) => {
  return (
    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
      ...
      {isWritten ? '✓ Written' : '✗ Missing'}
    </span>
  );
};

// Function to check if a blog post file exists
async function checkPostExists(slug: string, status: string): Promise<boolean> {
  try {
    if (status === 'published') {
      // Check if published post exists
      const publishedPath = path.join(process.cwd(), 'src', 'app', 'blog', slug, 'page.tsx');
      await fs.access(publishedPath);
      return true;
    } else {
      // Check if scheduled post exists
...
export default async function BlogCalendarPage() {
  // Check which posts are written
  const postsWithWrittenStatus = await Promise.all(
    scheduleData.schedule.map(async (post) => ({
      ...post,
      isWritten: await checkPostExists(post.slug, post.status)
    }))
  );

  // Group posts by month
  const postsByMonth: { [key: string]: typeof postsWithWrittenStatus } = {};
  
  postsWithWrittenStatus.forEach(post => {
  ...
  // Get stats
  const totalPosts = postsWithWrittenStatus.length;
  const publishedPosts = postsWithWrittenStatus.filter(p => p.status === 'published').length;
  const pendingPosts = postsWithWrittenStatus.filter(p => p.status === 'pending').length;
  const writtenPosts = postsWithWrittenStatus.filter(p => p.isWritten).length;
  const missingPosts = totalPosts - writtenPosts;
      ...
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <p className="text-gray-600 mb-6">
            Track and manage upcoming blog posts for Money in Motion
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-6">
            ...
            <div className="bg-emerald-50 p-4 rounded-lg">
              <p className="text-sm text-emerald-600">Written</p>
              <p className="text-2xl font-bold text-emerald-900">{writtenPosts}</p>
            </div>
            <div className="bg-red-50 p-4 rounded-lg">
              <p className="text-sm text-red-600">Missing</p>
              <p className="text-2xl font-bold text-red-900">{missingPosts}</p>
            </div>
                  ...
                  {posts.map((post) => {
                    ...
                    return (
                      <tr key={post.slug} className={isPast && post.status === 'pending' ? 'bg-red-50' : ''}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <WrittenBadge isWritten={post.isWritten} />
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
      ...
      <footer className="bg-gray-900 text-white mt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <p className="text-center text-gray-400">
            © 2025 Money in Motion. Internal tool - not for public access.
          </p>
        </div>
      </footer>
    </div>
